...
from miner.extractor.arinc424 import parse_navaids, parse_runway_thresholds  # noqa: E402
...
def main() -> int:
    ...
    if live:
        for f in live:
            ...
            runways = parse_runway_thresholds(f, args.airac)
            k = index.load(runways)
            total += k
            print(f"  NASR CIFP {f.name}: {k} runway thresholds (RWxx)")
    else:
